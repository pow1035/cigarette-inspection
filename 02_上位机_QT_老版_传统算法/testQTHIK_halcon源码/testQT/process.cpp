#include "process.h"
process::process(testQT* pDlg,int camera_number, uchar photo_number) {
...
void process::run()
{
	...
	while (testqt->systemRun)//设备运行
	{
		...
		if (!testqt->picQueList1_1.isEmpty())//队列有图像
		{
			ca_1_1_remain = testqt->picQueList1_1.size();//1组件相机1剩余待处理图像数量
		}
		else {
			ca_1_1_remain = 0;
		}
		...
		if (!testqt->picQueList2_2.isEmpty())//队列有图像
		{
			ca_2_2_remain = testqt->picQueList2_2.size();
		}
		else {
			ca_2_2_remain = 0;
		}
		*/
		...
		if (ca_1_1_remain!=0) 
		{
			...
			if (ca_1_1_remain == 1)
			{
				if (testqt->mutexPicQueList1.tryLock())//是否存图完毕
				{
					testqt->mutexPicQueList1.unlock();//解锁
				}
				else
				{
					Sleep(1);//正在存图，等1毫秒
			...
			picStruct ps = testqt->picQueList1_1.dequeue();//图片出队列
			...
			if (Bool_cig2RejectEnable)//烟支2剔除
			{
				...
				testqt->showNGPic1 = ho_cig2DrawNGSingle;//显示组件2缺陷图
			...
			if (result1)//第一支烟检测有问题
			{
				...
				if (Bool_cig1RejectEnable)//直接剔除
				{
					...
					testqt->showNGPic1 = ho_cig1DrawNGSingle;//显示组件1缺陷图
					picSaveStruct picSaveStruct1;
					picSaveStruct1.ho_double_gray_image = ps.ho_Cam_Image;//原始图
			...
			if (showNumber >= showIntervel)
			{
				...
				if (showCheckRegion)
				{
					testqt->showPic1 = ho_showCheckRegion;
				}
				else
				{
					testqt->showPic1 = ps.ho_Cam_Image;
				}
				showNumber = 0;
			}
...
bool process::whiteCigDarkLightCircleFilterProcess(picStruct& ps, bool *Bool_cig1RejectEnable, bool *Bool_cig2RejectEnable, uint *uint_cig1Result, uint *uint_cig2Result, HObject *ho_cig1DrawNGSingle, HObject *ho_cig2DrawNGSingle, uint8* uint8_cig2RejectNumberIO, HTuple* hv_cig1Param, HObject* ho_showCheckRegion)//处理过程：硬红
{
...
LONGLONG process::getExpendTime(LONGLONG startQpart)
{
