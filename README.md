...
- P8 测试集当前仍为 76 项：严格 package/主机报告 preflight 17、SDK-free soak 9、fixture release/原子激活回滚 17、Windows wrapper/collector 静态契约 2、package manifest generator 7、跨机 evidence verify/import 24。主机证据链已返修：v4 wrapper 每次生成 64 位小写十六进制密码学 challenge，复制可信 collector 到 `provenance/` 后以 mandatory `-RepositoryRoot $repoRoot` 立即现场采集 `p8-windows-host-report-v2` 与 `p8-gpu-host-report-v2`；RepositoryRoot 与 OutputDirectory 不得重叠，报告绑定 challenge、package manifest SHA、同一 capture/host/time 与 collector SHA，不再接受历史 `WindowsInput`/`GpuInput`。collector/wrapper 使用 ownership marker 并反复拒绝 reparse point，失败不递归删除目录；collector 不执行 PATH 中的 python/git/qmake/nvidia-smi，只读取应用版本、依赖文件大小/哈希，GPU 事实来自 CIM。可信 verifier 精确复验 preflight 顶层、claims、inputs 和 9 项成功检查，要求采集时间处于 wrapper 窗口，并要求 bundle 中 7 个 provenance 文件逐字节匹配当前受信仓库。v4 证据使用 Package、EvidenceRoot、DeploymentRoot 外的 exactly 32-byte key；manifest 以 `CreateNew + Flush(true)` 写入固定 UTF-8 bytes，SHA/HMAC 针对同一 bytes 计算并复读确认未漂移。跨机 verify/import 必须同时提供带外 manifest SHA、HMAC 和 bundle 外 key，receipt 为 v4。HMAC 只认证证据，不是产品验收。返修前 full gate 曾通过；返修后的最终 `./scripts/run_all_local_gates.sh --full` 与独立 review/QA 尚待执行。PowerShell 静态门计数仍为 13 个脚本、CLI 7/7。真实 Windows + Qt/HALCON/MVS/DAQNavi/CUDA/TensorRT/OpenCV、数据、许可和硬件证据仍缺失，P8 保持进行中。
