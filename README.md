...
- P8 Python 测试清单仍为 81 项：历史 76 项 package/Windows 工具门之外新增 5 项连续运行防假绿测试。当前候选已实跑连续专项 5/5 与 P8 精确 81/81；公开 `run_p8_local_continuous_soak.sh` 的 C++17 `contract-test-v1` 已完成 run + self-verify，并由独立 `verify` 再验。`contract-test-v1` 固定 0.12 秒、累计进程组 CPU 至少 0.01 秒、RSS 增长不超过 16 MiB；`local-sdkfree-v1` 固定 2×300 秒、60 秒 warm-up、每轮至少 240 个资源 samples、240 条 progress、最大 progress gap 5 秒、至少 240 sessions/122880 frames、累计进程组 CPU 至少 10 秒、RSS 增长不超过 64 MiB、磁盘余量至少 1 GiB。
...
- 当前树的 P8 精确 81/81、`--core` 和 `--full` 均已 PASS；full 另覆盖 C++14 contract run + verify、20 次优化仿真和 ASan/UBSan continuous-soak runtime。KI-048 已修复并经独立 reviewer 最终复核 PASS（P0/P1/P2/P3=0/0/0/0）：evidence 绑定新/旧两个 Python tool source 的 hash+snapshot，并固定 POSIX `ps` 的可信绝对路径、size/SHA、version probe、argv 与 binary snapshot；helper 漂移、PATH shadow 和依赖记录/快照篡改均拒绝。正式 `local-sdkfree-v1` 2×300 秒 evidence 已在 `artifacts/p8-continuous-local-20260727-final-v2` 完成 self-verify、主代理独立 verify 和 reviewer 两次独立 verify；两轮分别为 300.046591/300.020852 秒，共处理 3,793,408 帧，manifest SHA-256 为 `2bd420fab44acbed98315ffac5cf6a2b22567dcae42d98e522be35de3a7bcdd4`。正式 evidence QA/observability/cleanup PASS；唯一非阻断 P3 是旧 `invalid-dependency-gap` 失败目录仍在本地隔离保留，它无 manifest、被 Git 忽略，不得引用、混入正式证据或交付。P8 v2 本地连续工具证据可标 PASS，但真实 Windows/Qt/GPU/TensorRT/SDK/D 盘、正式数据、许可和硬件仍未验收，P8/AC-08 整体保持进行中。
...
GitHub Actions 的 `Local gates` 工作流会在 pull request、`main` push 和手动触发时执行文档/P1 静态门、P5/P6/P8 Python 精确计数回归、无 SDK C++ 回归和 PowerShell 静态门。当前工作树把 P8 计数更新为 81，并把公开 wrapper 驱动的受控 C++17 `LocalSoakRuntime.cpp` contract run、自验和独立二次 verify 接入 core；full 另覆盖受控 C++14 contract、20 次优化仿真与 ASan/UBSan runtime。当前本地 `--core`/`--full` 已 PASS，但该增量尚无 hosted PASS。提交 `7e7c2de2b157cf5c2ace8b5263e8db5f57c89902` 的在线运行 `30169095184`（job `89706968923`）SUCCESS、全部步骤通过、check-run annotations 为空、原 Node 20 warning 已消失，checkout/setup-python v6 均固定完整提交 SHA；最终 reviewer `019f9a6b-f76e-7620-a9e4-1e681e7f8d0b` 与 QA `019f9a6c-0733-7da1-986f-6176824be92d` PASS。该结果仅是 2026-07-26 的历史 76 项/CI 硬化快照。Windows/MSVC、Qt UI、TensorRT/GPU 和现场硬件仍保留为人工或目标机门禁，不能由 Linux CI 代替。
