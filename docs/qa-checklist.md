...
| `contract-test-v1` C++ 短语义门 | 本地/独立复核通过；hosted 复核通过 | hosted run `30219159920` 在 Linux 将 0.12 秒实际运行的 CPU 量化为 0.00 秒。修复提交 `cc7a3ab` 让 wrapper/direct project contract 实际运行 1.0 秒，locked minimum 0.12 秒、timeout 2.0 秒和其他阈值不变；Mac/Colima/Linux full 与独立 reviewer PASS；修复提交 hosted run `30221330296`（job `89844182732`）SUCCESS |
...
| hosted Linux `Local gates` | 修复后已通过 | `5f6a06a` → run `30219159920` / job `89838475434` FAIL，两项失败均为有效发现；当前修复 reviewer 与 QA/observability PASS，修复已提交并 push；第二次 hosted run `30221330296`（job `89844182732`）SUCCESS，GitHub API 返回的 10 个已执行步骤（编号 1–7、13–15）全部成功，check-run annotations 为空 |
