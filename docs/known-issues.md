...
| KI-050 | 高 | 提交 `5f6a06a` 的 hosted Linux 门发现短 project contract CPU 计时量化为 0.00 秒，且旧 POSIX abort fixture 被 core-dump 路径误分类 | 当前未提交修复让 public wrapper/direct project contract 实际运行 1.0 秒，同时保持 locked minimum 0.12 秒、timeout 2.0 秒及其他阈值不变；旧 crash fixture 在 POSIX abort 前设置 `RLIMIT_CORE=0`。Mac 与 Colima/Linux 原始 full、P8 81/81、独立 reviewer 和 QA/observability 均 PASS；提交/push 与 hosted rerun 待完成。正式 `final-v2` 绑定文件未变且 verify PASS | 修复候选待 hosted 复核 | P8 |
