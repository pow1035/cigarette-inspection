...
- 独立 PowerShell 静态门当前在本机 Colima/Linux arm64 使用 PowerShell 7.6.3、PSScriptAnalyzer 1.25.0 扫描 13 个 `.ps1`，parser/analyzer finding 均为 0；CLI 契约 7/7 PASS。结果固定 `windowsRuntimeClaimed=false`，不并入 P8 的 81 项 Python 清单。
...
- 新增 `scripts/p8_continuous_soak.py`：按同一进程组/进程树采样 RSS 与累计 CPU，计算 CPU P50/P95，检查时长、采样完整性、warm-up 后 RSS、磁盘、输出、超时、崩溃、残留进程和 evidence hash，并支持离线二次 verify。
...
- 该 PowerShell 增量时点的 P8 测试集为 76 项；PowerShell 7/7 单独记录。当前 P8 清单已扩展到 81 项且当前候选精确 81/81 PASS。2026-07-25 的最终 reviewer/QA 只覆盖历史 70 项，PowerShell reviewer/QA 只覆盖当时静态门快照，均不自动覆盖后续 challenge/HMAC/v2/v4 或连续运行增量。
...
# 2026-08-23 accelerated integration checkpoint

- Protected P5 dirty work on `codex/p5-protected-20260823` and integrated it in the isolated P8 worktree without commit, merge, push, or release.
- Added Windows Python-helper launch compatibility to the P8 evidence runners; Unix-shebang `.py` helpers are launched through the active interpreter on Windows.
- Progress timestamp validation is monotonic to tolerate Windows timer quantization while still rejecting time reversal.
- Combined P5/P6/P8 Python validation: `255 passed, 20 skipped, 137 subtests passed`. POSIX shell and abort fixtures are explicitly skipped on Windows; this is not Windows/GPU/现场验收 evidence.
- Documentation validation now selects an available `python3` or `python` interpreter.
# 2026-08-23 - Windows evidence hash guard

- Fixed a cross-platform false blocker in the P5 readiness gate: Windows `core.autocrlf` converted the class catalog from LF to CRLF while Git reported no content drift, changing its evidence SHA-256.
- Added `.gitattributes` LF policy for evidence-bound JSON and a regression test that checks the repository catalog bytes and digest.
- Fresh-clone readiness now exits 2 with exactly the two expected external artifact gaps (`reviewed_truth`, `fallback_baseline`); the catalog mismatch is closed. No external evidence was promoted.
- 2026-08-23 production acceptance acceleration: added `scripts/production_acceptance_status.py` and four regression tests. Fresh-clone status remains exit 2 with only `reviewed_truth` and `fallback_baseline` absent; next action is controlled recovery plus external evidence-manifest SHA-256 comparison. No controlled artifact was modified and no commit, push, or release was performed.
- 2026-08-24 recovery acceleration: added `scripts/p5_verify_external_digest.py` with match/mismatch/missing/invalid exit semantics and four regression tests. It is read-only and does not replace controlled-channel trust.
- 2026-08-24 Windows Release build: temporarily added `E:\visual studio\MSBuild\Current\Bin` to the process PATH and ran `scripts/run_windows_p1_build.ps1 -Configuration Release`. After converting `MultipleCameraDefine.h` from the local code page to UTF-8, `CigVision.exe` and `process.dll` built successfully with no C4828 warnings. Evidence: `artifacts/p1-windows-20260824-release-utf8`.
- 2026-08-24 Windows startup smoke: launched the freshly built Release executable in its output directory with the default safe configuration, observed it alive for 5 seconds, then stopped only the harness-owned process. Evidence: `artifacts/p1-windows-20260824-release-utf8/startup-smoke.json`; startup-only, not Qt workflow, GPU, camera, IO, or production acceptance.
- 2026-08-24 TensorRT 10 recovery and fixed-input run: rebuilt `yanzhi20260115.onnx` with TensorRT 10.15.1 on RTX 4060 Laptop (CC 8.9), producing engine SHA-256 `9E54C0543F2B797D067ABA68C6530FF62861FF00E6DDA0C5D2CB5B6B2EF34A43`. `scripts/run_windows_p4_tensorrt.ps1` completed with exit 0 in `artifacts/p4-tensorrt-20260824-trt10-rebuild`: 116/116 processed, detector/source/observer/save errors 0, dropped 0, NG 88, OK 28. Invalid config/shape and malformed/conflicting CLI negative paths returned 4/4/2/2. This is TensorRT execution evidence only: `groundTruthAvailable=false`, `accuracyMetricsClaimed=false`, `realRejectEnabled=false`; no commercial metric or production acceptance claim.
- 2026-08-24 Windows P2/P7 acceleration: `run_windows_p2_contract_tests.ps1` initially exposed a relative `-EvidenceRoot` path bug (build succeeded but output checks looked under the wrong working directory). The script now canonicalizes `EvidenceRoot` before MSBuild; rerun evidence `artifacts/p2-contracts-20260824-accelerated-v2` passes Debug/Release 7/7 with exit 0. `CigVision.ProductState.vcxproj` Release rebuild and executable run pass 8/8. This is SDK-free contract/state evidence, not Qt UI or real-device acceptance.
- 2026-08-24 Windows P3 acceleration: `scripts/run_windows_p3_offline.ps1` completed in `artifacts/p3-offline-20260824-accelerated`. Debug/Release offline tests both pass 7/7; main Qt Release rebuild passes; fixed 8-frame batch completes with processed=8, OK=4, NG=4, errors/drops/save failures=0; invalid manifest exits 2; startup window capture and observation JSON are recorded. This proves the offline Qt/product file path only and does not prove GPU, camera, DAQNavi, IO, or production acceptance.
- 2026-08-24 P6/P8 acceleration: `artifacts/p6-windows-simulation-20260824-accelerated` passed Release rebuild, 4/4 fixture frames/PNGs, trace/summary/hash checks, and six CLI rejection paths. The manifest keeps real hardware, real IO/reject, accuracy, and TensorRT claims false.
- 2026-08-24 regression: `python -m pytest -q tests/p8 tests/p6` -> `87 passed, 12 skipped, 80 subtests passed`; compileall and `git diff --check` passed. Native MSVC contract soak passed in `artifacts/p8-msvc-contract-20260824-v9` with runtime/resource gates and self-verify. The locked `local-sdkfree-v1` soak, shell validator, and production Windows/GPU/Qt/DAQNavi/IO gates remain separate and unverified on this host.
- Production status remains blocked on controlled P5 artifacts, reviewed truth/approved metrics, GPU/deployment/soak, Qt QA, real camera/DAQNavi/IO/safety, and three-party sign-off.
