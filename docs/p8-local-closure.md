...
→ hosted Linux 首轮发现两项短门跨平台缺口；修复提交 `cc7a3ab` 的 Mac/Linux full、reviewer 与 QA/observability PASS，hosted run `30221330296`（job `89844182732`）SUCCESS，GitHub API 返回的 10 个已执行步骤（编号 1–7、13–15）全部成功，check-run annotations 为空
```

历史本地工具通过状态写作 `passed-local-tooling`；当前 P8 v2 正式 manifest 写作 `passed-local-continuous-tooling`。短 contract、当前 Mac/Colima/Linux 综合本地门、KI-048 dependency provenance、正式两轮 evidence、独立 verify、当前 CI 修复 reviewer/QA/observability 和正式 evidence QA/observability/cleanup 均已通过，因此 P8 v2 本地连续工具证据可以标 PASS。hosted run `30221330296`（job `89844182732`）SUCCESS，GitHub API 返回的 10 个已执行步骤（编号 1–7、13–15）全部成功，check-run annotations 为空；它仍不等于 AC-08 或 P8 整体通过，不证明 Windows/Qt 产品、TensorRT/GPU、D 盘安装、生产吞吐、商业准确率、真实磁盘满恢复或现场 IO/剔除。
...
- 当前修复门：`5f6a06a` hosted run `30219159920` 有效发现 Linux CPU tick/core-dump 两项缺口；修复提交 `cc7a3ab` 现已取得 Mac 与 Colima/Linux 原始 full、P8 81/81、独立 reviewer 和 QA/observability PASS。修复提交已 push，hosted run `30221330296`（job `89844182732`）SUCCESS，GitHub API 返回的 10 个已执行步骤（编号 1–7、13–15）全部成功，check-run annotations 为空。
- 独立 PowerShell 静态门：13 个 `.ps1` parser/analyzer 0 finding；PowerShell CLI 契约 7/7 PASS（不计入上述 81 项清单）。

2026-07-27 KI-048 与正式 `final-v2` 2×300 秒、独立 verify、正式 evidence QA/observability/cleanup 均已通过。提交 `5f6a06a` 的 hosted run `30219159920` 随后发现短 contract CPU tick 和 POSIX core-dump fixture 两项跨平台缺口；修复提交 `cc7a3ab` 的 Mac/Linux full、独立 reviewer 与 QA/observability 已通过，hosted 修复 run `30221330296` 已收口。因此正式本地连续 evidence 结论保持有效，实现提交门已通过；P8/AC-08 整体继续进行中。
