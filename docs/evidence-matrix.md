...
| AC-08-02 | package 与现场主机报告可由外部 manifest SHA、challenge、逐文件哈希及严格 schema/同源身份绑定预检 | `scripts/p8_preflight.py`、`config/p8-local-gates-v1.json`、`tests/p8/test_p8_preflight.py` 17 项 | 进行中（返修后最终门待跑） | v2 Windows/GPU 报告必须绑定本次 64-hex challenge、package manifest、同 capture/host/time/collector SHA、精确检查集全部 passed，且 acceptance/runtime/IO/reject claims 为 false；ready 不等于产品语义验收 |
...
| AC-08-02 | Windows collector/wrapper 固定 D 盘并串联现场主机采集、预检、soak、fixture 发布和可选回滚 | `scripts/collect_windows_p8_host_reports.ps1`、`scripts/run_windows_p8_preacceptance.ps1`、`tests/p8/test_p8_windows_wrapper.py` 2 项 | 进行中（返修后最终门待跑） | v4 wrapper 生成 challenge、复制 collector 后立即子进程采集 v2 报告；collector 只读版本/CIM/依赖文件大小与哈希，不执行 PATH 工具；两层 ownership/reparse 防护失败不递归删目录。Windows/D 盘实跑仍缺失 |
...
| AC-08-03 | Windows evidence v4 可移植验真并原子导入 | `scripts/p8_windows_evidence_verify.py`、`tests/p8/test_p8_windows_evidence_verify.py` 24 项、`docs/windows-target-execution.md` | 进行中（返修后最终门待跑） | 外部 manifest SHA + HMAC + exactly 32-byte key 共同认证 manifest；key 不得进入 Package/EvidenceRoot/DeploymentRoot/bundle。manifest 以 `CreateNew + Flush(true)` 写入固定 UTF-8 bytes，SHA/HMAC 针对同一 bytes 计算并复读确认未漂移；verifier 精确复验 preflight 9 项、v2 host input、采集窗口与 7 个受信 provenance；receipt v4 保持 `productAcceptanceClaimed=false` |

主代理定向复核为 32/32。

P8 测试集当前仍为 76 项（2026-07-26）：preflight 17、soak 9、release 17、wrapper/collector 2、package manifest 7、evidence verify/import 24。返修前 full gate 曾通过；challenge/HMAC/v2/v4 返修后的最终 `./scripts/run_all_local_gates.sh --full`、独立 reviewer/QA 尚待执行，因此当前不得把 76 项写成最终提交门 PASS。无论本地门结果如何，都不覆盖目标 Windows/Qt/GPU/D 盘、真实数据、许可或硬件。

PowerShell 静态门增量（2026-07-26）：在本机 Colima/Linux arm64 中使用 PowerShell 7.6.3 与 PSScriptAnalyzer 1.25.0 执行 `validate_powershell_scripts.ps1 -RequireScriptAnalyzer`，当前扫描 `scripts/` 与 `tests/powershell/` 共 13 个 `.ps1`，parser finding 0、analyzer finding 0。门禁要求 PSScriptAnalyzer 最低 1.25.0，并显式启用 `PSUseCompatibleSyntax`，机器可读结果记录 `syntaxCompatibilityTargets=["5.1"]`，目标为 Windows PowerShell 5.1。`test_validate_powershell_scripts.ps1` 的 7/7 CLI 契约保持通过。独立 reviewer/QA 结论覆盖静态门设计和当时 12 脚本快照；新增 collector 已由当前静态门扫描为零 finding，但不把历史评审扩写到主机报告产品语义。工作流已接线，没有 GitHub Actions 在线运行证据。
