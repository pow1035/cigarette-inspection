...
| 验收 ID | 声明 | 证据 | 状态 | 限制 |
| --- | --- | --- | --- | --- |
| AC-08-01 | 旧 SDK-free soak 编排可记录多轮重启、超时/崩溃、RSS、磁盘和输出量 | `scripts/p8_soak_evidence.py`、`tests/p8/test_p8_soak.py` 9/9、`docs/p8-local-closure.md` | 进行中（历史本地子门通过） | 这是 Windows v1 证据链的短编排契约；缺少最低持续时长/采样和同一进程 RSS 增长语义，不能证明连续稳定性，也不由 P8 v2 改写 |
| AC-08-01 | P8 v2 可拒绝瞬时成功假绿，并对真实 SDK-free runtime 形成固定时长、同一进程组资源与业务守恒证据 | profile/tool/public wrapper/C++ runner/5 项测试；连续专项 5/5、P8 精确 81/81、C++17 wrapper run + self-verify + 独立 verify；当前返修的 Mac/Colima/Linux core/full PASS，独立 reviewer P0/P1/P2/P3=0/0/0/0。正式 evidence `artifacts/p8-continuous-local-20260727-final-v2`：2/2 runs、300.046591/300.020852 秒、291/292 samples、30.64/23.51 CPU 秒、RSS +16 KiB/0、progress 3786/3623、gap 0.085584/0.098617 秒、3,793,408 frames；manifest SHA-256 `2bd420fab44acbed98315ffac5cf6a2b22567dcae42d98e522be35de3a7bcdd4`，29 个 inventory 文件/3,654,771 bytes。self-verify、主代理独立 verify、reviewer 两次独立 verify、正式 evidence QA/observability/cleanup 均 PASS；KI-048 toolDependencies 与负路径已独立复核 | 进行中（P8 v2 本地连续工具证据通过；hosted rerun pending） | 本地 manifest/verify 无带外 HMAC，只证明包内一致性与 snapshot/源码绑定，不能认证协调重写整包；不证明 Windows/GPU/真实 IO/产品验收。旧无 manifest 失败目录仅隔离保留作失败审计，不计 evidence、不引用或交付 |
...
2026-07-27 当前增量：连续运行专项 5/5、P8 精确 81/81、公开 C++17 wrapper run + self-verify + 独立 verify，以及当前返修的 Mac/Colima/Linux `--core`/`--full` 均 PASS。返修把 project compile 收进 `run` 的受控路径，外部 provenance 限于 helper，并覆盖伪造 project provenance 拒绝；另增加 verified `durationSeconds`/真实 `processLifetimeSeconds`、Linux `/proc` CPU 与 pure-sleep 负路径、持续 progress、ProductRuntimeState OK/NG/Error、严格 evidence inventory、Windows 两次 fresh tree confirmation。当前 CI 修复独立 reviewer 与 QA/observability 均 PASS，P0/P1/P2/P3=0/0/0/0；正式 `final-v2` 两轮 evidence、独立 verify 和既有 QA/observability/cleanup 均 PASS。hosted rerun pending；P8/AC-08 整体仍受 Windows/Qt/GPU/D 盘、正式数据、许可和硬件条件阻断。

历史快照：HEAD `6886856` 最终 full gate（2026-07-26）PASS：P5 100/100、P6 17/17、P8 76/76（preflight 17、soak 9、release 17、wrapper/collector 2、package manifest 7、evidence verify/import 24）、C++14/C++17、20 次重复、ASan/UBSan；PowerShell 13 个脚本 parser/analyzer 0 finding、CLI 7/7。该结论不覆盖当前连续运行增量，也不覆盖目标 Windows/Qt/GPU/TensorRT/SDK/D 盘、真实数据、许可或硬件；P5 受控 artifact 仍缺失。
...
上一成功 hosted 基线（2026-07-26）：提交 `6d492528c7f9c0b0b2e3cc70e1c60a74cb62cede` 的 `Local gates` run `30188084112`（job `89756233568`）SUCCESS。当前最新推送 `5f6a06a` / run `30219159920`（job `89838475434`）FAIL，返修 rerun pending；两者均不覆盖目标机 runtime。
