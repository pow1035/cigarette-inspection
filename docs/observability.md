...
P3/P4 已具备逐帧 JSON、汇总计数、输入/输出哈希和 detector latency 证据；P6/P7 已形成 SDK-free 仿真与产品状态指标。P8 v2 增加同一进程组 RSS/累计 CPU/P50/P95、持续 progress、磁盘、输出、verified runtime 工作时长、进程寿命、受控 build/tool provenance、ProductRuntimeState 分桶和严格 inventory。当前连续专项 5/5、P8 精确 81/81、公开 wrapper C++17 contract run/self-verify/独立 verify，以及 Mac/Colima/Linux 原始 core/full 均 PASS；当前 CI 修复 reviewer 与 QA/observability 均 PASS，P0/P1/P2/P3=0/0/0/0。正式 `artifacts/p8-continuous-local-20260727-final-v2` 两轮 evidence、主代理/reviewer 独立 verify 和正式 evidence QA/observability/cleanup 均 PASS；hosted rerun 尚待完成，该结论仅覆盖 P8 v2 本地 SDK-free 连续工具证据。
...
上一成功 hosted 基线为提交 `6d492528c7f9c0b0b2e3cc70e1c60a74cb62cede` 的 `Local gates` run `30188084112`（job `89756233568`）SUCCESS。当前最新推送 `5f6a06a` 的 run `30219159920`（job `89838475434`）FAIL，返修 rerun pending；边界仍为 `windowsRuntimeClaimed=false`、`productAcceptanceClaimed=false`。
