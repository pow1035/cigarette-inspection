...
- 锁定 profile：短门最低仍为 0.12 秒、CPU ≥0.01 秒、progress ≥2/gap ≤0.5 秒、RSS ≤16 MiB、timeout 2.0 秒；修复提交 `cc7a3ab` 仅让 public wrapper/direct project contract 实际运行 1.0 秒，其他阈值与 CLI 锁定不变。正式门仍为 2×300 秒、330 秒 timeout、60 秒 RSS warm-up、每轮 samples/progress ≥240、gap ≤5 秒、sessions ≥240、frames ≥122880、CPU ≥10 秒、RSS ≤64 MiB、磁盘 ≥1 GiB。
...
- 修复提交 `cc7a3ab`：public wrapper/direct project contract 实际执行 1.0 秒；旧 crash fixture 在 POSIX abort 前设置 `RLIMIT_CORE=0`。Mac 与 Colima/Linux 原始 `--full`、P8 精确 81/81、修复提交 CI reviewer 与 QA/observability 均 PASS，P0/P1/P2/P3=0/0/0/0；修复提交 hosted run `30221330296`（job `89844182732`）SUCCESS，GitHub API 返回的 10 个已执行步骤（编号 1–7、13–15）全部成功，check-run annotations 为空。
...
- 正式 `final-v2` 仍可离线 verify，因为其绑定的 `p8_continuous_soak.py`、profile、C++ runtime、旧 helper/core headers 未改。修复提交 full/reviewer/QA/observability 已 PASS，但 hosted run `30221330296`（job `89844182732`）SUCCESS，实现提交门已通过。
...
- Limits: this historical snapshot did not prove a hosted Ubuntu run and had not pinned action dependencies; subsequent commit `7e7c2de2b157cf5c2ace8b5263e8db5f57c89902` pinned both actions and passed hosted run `30169095184`. Windows/MSVC, TensorRT/GPU, and hardware behavior remain outside that evidence.
